...
From pySlitMask.py by Steve Crawford, slightly modified here by Matt Hilton
...
#        print 'the creator is: ', c
    ...
    def outFoV_row(self,i):
        ...
        xp,yp = wcs.wcs_world2pix(self.slitlets.data[i]['targ_ra'],\
        ...
        #print(self.slitlets.data[i]['name'], pass_test)

        # set the FoV flag for slits inside the FoV
        try:
            self.slitlets.data[i]['fov_flag'] = 1 * pass_test
        except:
            print("huh")
            import IPython
            IPython.embed()
...
#        xleft = 1
        ...
        # convert onsky coords to pix
        xp, yp = wcs.wcs_world2pix(self.slitlets.data['targ_ra'][is_in_mask],
...
#        tx1 = xp + xright + (xpad / 2.) # [ careful switching between lower left and width and lower left and upper right notation ]
        ...
        for i in range(nslits):
            ...
            if self.slitlets.data['collision_flag'][is_in_mask[i]]:
               print("update the code for resolving a collision")
               import IPython
               IPython.embed()
               sys.exit()
...
#    def FOVTest(cra,cdec,equinox,rotang,slitra,slitdec,slit_length,tilt):
    def outFoV_all(self):
        ...
        # convert onsky coords to pix
        xp, yp = wcs.wcs_world2pix(self.slitlets.data['targ_ra'],
